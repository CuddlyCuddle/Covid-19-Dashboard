...
```{r, echo = F}
cum_dat <- data %>% group_by(location) %>%
...
by_days <- data %>% group_by(location) %>% filter(total_cases >= 1, location != "World") %>%
  ...
  select(location, total_cases, days, population)
```
...
```{r}
fig <- plot_ly(by_days[by_days$location %in% c( , ),], x = ~days, y = ~total_cases, color = ~location, type = "scatter", mode = "lines")
